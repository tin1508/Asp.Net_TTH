...
{
    Task<StationeryOrder> CreateNewOrder(Cart cart);
    Task<StationeryOrder> CreateSingleItemOrder(string userId, int stationeryId, int quantity);
    Task<List<OrderListItemViewModel>> GetAllOrderListAsync();
    Task<List<OrderListItemViewModel>> GetOrderHistoryByUserIdAsync(string userId);
}
public class OrderStationeryService : IOrderStationeryService
{
    ...
    public async Task<StationeryOrder> CreateNewOrder(Cart cart)
    {
        if(cart.Items is null || !cart.Items.Any()) throw new AppException(ErrorCode.EMPTY_CART_ITEM);
        var order = new StationeryOrder
        {
            UserId = cart.UserId,
            CreatedAt = DateTime.UtcNow
        };
        foreach (var item in cart.Items.Where(i => i.StationeryId > 0 && i.Quantity > 0)){
    ...
    public async Task<StationeryOrder> CreateSingleItemOrder(string userId, int stationeryId, int quantity)
    {
        if (quantity <= 0)
        {
            throw new AppException(ErrorCode.AT_LEAST_STATIONERY_WHEN_ORDERING);
    ...
    public async Task<List<OrderListItemViewModel>> GetOrderHistoryByUserIdAsync(string userId)
    {
        var orders = await _stationeryOrderRepository.GetByUserIdAsync(userId);
        return orders.Select(o => new OrderListItemViewModel
        {
            Id = o.Id,
            CreatedAt = o.CreatedAt,
