...
public class StationeryRepository : Repository<Stationery, int>, IStationeryRepository
{
    ...
    public async override Task<Stationery?> GetByIdAsync(int id)
    {
        return await _context.Stationeries.Include(s => s.Category).FirstOrDefaultAsync(s => s.Id == id);
    }
    public async override Task<List<Stationery>> GetAllAsync()
    {
        return await _context.Stationeries.Include(s => s.Category).Where(s => !s.IsDeleted).ToListAsync();
    }
}